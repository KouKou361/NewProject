...
Lib_LambertShader::Lib_LambertShader(ID3D11Device* device)
{
	HRESULT hr = S_OK;
	//シェーダー
	m_shader = make_unique<ShaderResource>();
	m_shader->Create(device, "./Shader/Lambert_vs.cso", "./Shader/Lambert_ps.cso", ShaderResource::TYPELayout::TYPE_LayoutModel);
...
void Lib_LambertShader::SetConstant(DirectX::XMFLOAT4 color)
{
	constant.Color = color;
}

void Lib_LambertShader::Begin(
...
void Lib_LambertShader::Render(ID3D11DeviceContext* context, const ModelResource* model)
{
...
void Lib_LambertShader::End(ID3D11DeviceContext* context)
{
