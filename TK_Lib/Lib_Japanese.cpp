...
JapanFont::JapanFont(ID3D11Device* device, const char* filename, int maxSpriteCount)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\JapanFont_VS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\JapanFont_PS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
...
void JapanFont::Begin(ID3D11DeviceContext* context)
{
	// スクリーンサイズ取得
...
void JapanFont::Draw(float x, float y, const wchar_t* string)
{
...
void JapanFont::End(ID3D11DeviceContext* context)
{
	// 頂点編集終了
