...
SoundSource* SoundManager::CreateSoundSource(const char* filename)
{
	return new SoundSource(directSound, filename);
}
...
//名前からサウンドを取得
SoundSource* SoundManager::GetSound(const char* KeyName)
{
	return Sounds.at(KeyName);
}
//サウンドの登録
void SoundManager::RegisterSound(const char* fileName, const char* RegisterName)
{
	for (auto& sound : Sounds)
	{
		if (strcmp(RegisterName, sound.first) == 0)
			assert(!"登録しようとしたRegisterNameと要素の中のRegisterNameがかぶっています！");
	}
	SoundSource* sound =CreateSoundSource(fileName);
	Sounds.insert(make_pair(RegisterName, sound));
}

//登録されているサウンドを全て削除
void SoundManager::SoundClear()
{
	for (auto& sound : Sounds)
	{
		SoundSource* soundsource = sound.second;
