...
Lib_OutlineShader::Lib_OutlineShader(ID3D11Device* device)
{
	HRESULT hr = S_OK;
	//シェーダー
	m_shader = make_unique<ShaderResource>();
	m_shader->Create(device, "./Shader/Lib_Outline_vs.cso","./Shader/Lib_Outline_gs.cso", "./Shader/Lib_Outline_ps.cso", ShaderResource::TYPELayout::TYPE_LayoutModel);
...
void Lib_OutlineShader::SetConstant(DirectX::XMFLOAT4 color)
{
	constant.Color = color;
}

void Lib_OutlineShader::Begin(
	...
	const DirectX::XMFLOAT4X4& projection, const DirectX::XMFLOAT4& lightDirection)
{
	...
	//context->OMSetDepthStencilState(DepthStencil::Instance().m_depthState[DepthStencil].Get(), 0);
...
void Lib_OutlineShader::Render(ID3D11DeviceContext* context, const ModelResource* model)
{
...
void Lib_OutlineShader::End(ID3D11DeviceContext* context)
{
